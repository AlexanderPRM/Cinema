...
async def test_film(
    ...
    resp = await make_get_request("/api/v1/films/", query_data)
...
async def test_id_film(
    ...
    # Делаем запрос к API с предписанными параметрами
    resp = await make_get_request("/api/v1/films/%s" % id_query_data["ID"])
    body = await resp.json()
    status = resp.status

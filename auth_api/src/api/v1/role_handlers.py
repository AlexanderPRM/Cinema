...
def get_roles():
    data = _decode_jwt_from_request(locations="json", fresh=False)
...
def create_role():
    ...
    if db.session.query(UserRole).filter_by(name=name).first():
        return abort(Response(json.dumps({"message": f"Role {name} already exists."}), 422))
...
def delete_role(id):
    ...
    if not role:
        return abort(Response(json.dumps({"message": f"Role {id} doesn't exists."}), 422))

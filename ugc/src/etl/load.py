...
class Loader:
    ...
    def load_data_to_ch(self, data, table_name=ch_config.CLICKHOUSE_TABLE_NAME, batch_size=1000):
        columns = ("user_id", "movie_id", "updated_at", "timestamp")
        query = "INSERT INTO {}.{} ({}) VALUES".format(
            "default", table_name, ', '.join(columns)
        )
        values_list = list()
        for item in data:
            if type(item) != Entry:
                logging.info("ClickHouse saving method get invalidated data: type={}, data={}".format(type(item), item))
                continue
            values = "('{}','{}','{}', '{}')".format(
